...
class FuseConfig:
    ...
    fusion_fn: str = field(default=None, metadata={"help": "Define fusion function applied in the adapter bottleneck, supported fusion functions are 'add', 'add_relu', 'mul', and 'cross-attention'."})
    cross_attention_config: dict = field(default=None, metadata={"help": "Configuration object for cross-atention module."})
...
class LoraConfig(PeftConfig):
    ...
    # dict type is used when loading config.json
    fuse_config: Union[FuseConfig, dict] = field(
        default_factory=dict,
        metadata={
            "help": (
                "Fusion configuration for the model. Used to specify the fusion of source and target language input within the Lora adapter."
            )
        },
    )
