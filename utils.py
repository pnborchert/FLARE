# paste generation mixin here: trainsformer.generation.utils.py
# coding=utf-8
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            # subset x_source (or x_source_decoder) corresponding to the current input_ids
            # if cur_len exceeds x_source length, the last token is used
            if "x_source" in model_kwargs:
                idx_tok_source = min(cur_len-1, len(model_kwargs["x_source"])-1)
                model_inputs["x_source"] = model_kwargs["x_source"][idx_tok_source]

                if (model_inputs["x_source"][-1] is not None) and (model_inputs["input_ids"].shape[1] < model_inputs["x_source"][-1].shape[1]):
                    model_inputs["x_source"] = tuple([x[:, -1:, :] for x in model_inputs["x_source"]])
