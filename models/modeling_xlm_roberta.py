...
# from transformers.models.deprecated._archive_maps import XLM_ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST  # noqa: F401, E402
    ...
    def forward(
        ...
        **kwargs,
        ...
        if x_source is not None:
            mixed_query_layer = self.query(hidden_states, x_source)
        else:
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states, x_source))
        else:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
