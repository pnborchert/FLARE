...
class LoraLayer(BaseTunerLayer):
    ...
    def loftq_init(self, adapter_name):
        from peft_local.utils.loftq_utils import loftq_init
...
class FuseCrossAttention(nn.Module):
    def __init__(self, config):
        ...
        self.attention_head_size = config.get("hidden_size") // self.num_attention_heads
...
class LinearFuse(Linear):
    """Linear with mutliple inputs and fusion function mixing inputs after lora_a and before lora_b"""
    ...
    def __init__(
        ...
        use_rslora: bool = False,
        use_dora: bool = False,
        fusion_fn: Optional[nn.Module] = None,
        **kwargs,
    ) -> None:
        super().__init__(
            base_layer,
        ...
        if fusion_fn == "cross-attention":
            self.fuse_cross_attention = FuseCrossAttention(config=kwargs.get("cross_attention_config"))
    ...
    def forward(
            self, 
            x_target: Optional[torch.LongTensor] = None,
            x_source: Optional[torch.LongTensor] = None,
            *args: Any,
            **kwargs: Any) -> torch.Tensor:
        
        self._check_forward_args(x_source, x_target, *args, **kwargs)
        adapter_names = kwargs.pop("adapter_names", None)
        ...
        if self.disable_adapters:
            ...
            result = self._mixed_batch_forward(x_source, *args, adapter_names=adapter_names, **kwargs)
        elif self.merged:
            result = self.base_layer(x_target, *args, **kwargs)
        else:
            result = self.base_layer(x_target, *args, **kwargs)
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    A_target = lora_A(dropout(x_target))
                    ...
                    if (x_source is not None) and (self.fusion_fn is not None) and (self.inference_mode == False):
                        ...
                        A_source = lora_A(dropout(x_source))
                        result = result + lora_B(self.fusion_fn(A_source, A_target)) * scaling

                    else:
                        result = result + lora_B(A_target) * scaling

                else:
                    x_target = x_target.to(lora_A.weight.dtype)
                    x_target = dropout(x_target)
                    result = result + self._apply_dora(x_target, lora_A, lora_B, scaling, active_adapter)
...
def dispatch_default(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        if lora_config.fuse_config is not None:
            kwargs.update(lora_config.fuse_config.asdict())
            new_module = LinearFuse(target, adapter_name, **kwargs)
        else:
